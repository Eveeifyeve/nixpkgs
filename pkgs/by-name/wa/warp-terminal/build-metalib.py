#!/usr/bin/env python3
"""
Build warp-terminal.metallib from the upstream shaders.metal source using Apple's
...
Usage:
  ...
  build-metallib.py --source PATH         # use a local warp-terminal checkout
...
PKG_DIR = HERE.parent
SHADER_REL = "crates/warp-ui/src/renderer/shaders/shaders.metal"
DEFAULT_OUTPUT = PKG_DIR / "warp-terminal.metallib"
MIN_MACOS = "10.14"
...
def compile_metallib(shader: pathlib.Path, output: pathlib.Path) -> None:
    with tempfile.TemporaryDirectory() as td:
        ir = pathlib.Path(td) / "warp-terminal.ir"
        run(
            [
                "xcrun",
                "-sdk",
...
def main() -> None:
    ...
    p.add_argument(
        "--source", type=pathlib.Path, help="path to a local warp-terminal checkout"
    )
    p.add_argument(
        "--output",
        type=pathlib.Path,
    ...
    if args.source is not None:
        ...
        tmp_root = pathlib.Path(tempfile.mkdtemp(prefix="warp-terminal-metallib-"))
        src_root = fetch_source(version, tmp_root / "warp-terminal")
